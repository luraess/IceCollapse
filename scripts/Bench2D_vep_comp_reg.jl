...
@views function update_velocities!((;Vx,Vy,Pr_c,τxx,τyy,τxyv,dτ_ρx,dτ_ρy,η),dx,dy)
    Vx[2:end-1,:] .+= dτ_ρx.*(diff(.-Pr_c.+τxx,dims=1)./dx .+ diff(τxyv[2:end-1,:],dims=2)./dy)
    Vy[:,2:end-1] .+= dτ_ρy.*(diff(.-Pr_c.+τyy,dims=2)./dy .+ diff(τxyv[:,2:end-1],dims=1)./dx)
    return
end
