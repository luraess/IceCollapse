...
@views function compte_η_G_ρg!((;η,G,ρgy_c,phase,ηb),η0,G0,ρg0,xc,yc,x0,y0,normx,normy,r_ice)
    Threads.@threads for iy in axes(η,2)
        for ix in axes(η,1)
            sd_rock  = normx*(xc[ix]-x0) + normy*(yc[iy]-y0)
            sd_ice   = max(sqrt((xc[ix]-x0)^2 + (yc[iy]-y0)^2)-r_ice, -sd_rock)
            t_rock   = 0.5*(tanh(-sd_rock/0.05) + 1)
            t_ice    = 0.5*(tanh(-sd_ice /0.05) + 1)
            t_air    = 1.0 - t_rock - t_ice
            η[ix,iy]     = t_rock*η0.rock  + t_ice*η0.ice  + t_air*η0.air
            G[ix,iy]     = t_rock*G0.rock  + t_ice*G0.ice  + t_air*G0.air
            ρgy_c[ix,iy] = t_rock*ρg0.rock + t_ice*ρg0.ice + t_air*ρg0.air
...
function main()
    # physics
    lx,ly       = 20.0,10.0
    η0          = (ice = 1.0  , rock = 1e6  , air = 1e-10)
    G0          = (ice = 1e20 , rock = 1e20 , air = 1e20 )
    ρg0         = (ice = 0.9  , rock = 2.0  , air = 0.0  )
    α           = π/12
    r_ice       = 0.4*min(lx,ly)
    x0,y0       = 0.0,0.4*ly
    normx,normy = sin(α),cos(α)
    dt          = 1.0
    # numerics
    nx       = 250
    ...
    # initialisation
    compte_η_G_ρg!(fields,η0,G0,ρg0,xc,yc,x0,y0,normx,normy,r_ice)
    ...
    # visualisation
    fig     = Figure(fontsize=32,resolution=(3500,1200))
    ax      = ( Pr    = Axis(fig[1,1][1,1],title="pressure" ,xlabel="x",ylabel="y",aspect=DataAspect()),
                τII   = Axis(fig[1,2][1,1],title="τII"      ,xlabel="x",ylabel="y",aspect=DataAspect()),
                Vmag  = Axis(fig[1,3][1,1],title="|v|"      ,xlabel="x",ylabel="y",aspect=DataAspect()),
                η     = Axis(fig[2,1][1,1],title="log10(η)" ,xlabel="x",ylabel="y",aspect=DataAspect()),
                phase = Axis(fig[2,2][1,1],title="phase"    ,xlabel="x",ylabel="y",aspect=DataAspect()),
                errs  = Axis(fig[2,3][1,1],title="residuals",xlabel="iter/nx",ylabel="ε",yscale=log10), )
    heatmaps = (
        Pr    = heatmap!(ax.Pr   ,xc,yc,fields.Pr   ;colormap=:turbo,),
        τII   = heatmap!(ax.τII  ,xc,yc,fields.τII  ;colormap=:turbo,),
        Vmag  = heatmap!(ax.Vmag ,xc,yc,fields.Vmag ;colormap=:turbo,),
        η     = heatmap!(ax.η    ,xc,yc,fields.η    ;colormap=:turbo,),
        phase = heatmap!(ax.phase,xc,yc,fields.phase;colormap=:turbo,),
    )
    for prop in propertynames(heatmaps)
        contour!(getproperty(ax,prop),xc,yc,fields.phase;levels=0.5:0.5,color=:white,linewidth=4)
    end
    cbars = (
        Colorbar(fig[1,1][1,2],heatmaps.Pr   ;),
        Colorbar(fig[1,2][1,2],heatmaps.τII  ;),
        Colorbar(fig[1,3][1,2],heatmaps.Vmag ;),
        Colorbar(fig[2,1][1,2],heatmaps.η    ;),
        Colorbar(fig[2,2][1,2],heatmaps.phase;),
    )
    # return cbars
    err_plots = ( lines   = [lines!(  ax.errs,Point2.(iter_evo,errs_evo[ip,:])) for ip in size(errs_evo,1)],
                  scatter = [scatter!(ax.errs,Point2.(iter_evo,errs_evo[ip,:])) for ip in size(errs_evo,1)], )
    display(fig)
    # time loop
    for it = 1:nt
        @printf("it=%d\n",it)
        # initialise time step
        fields.τxx_old .= fields.τxx
        fields.τyy_old .= fields.τyy
        fields.τxy_old .= fields.τxy
        ...
        while any(errs .>= ϵtol) && iter <= maxiter
            ...
            if iter % ncheck == 0
                ...
                errs = (
                    maximum(abs.(fields.r_Vx)),
                    maximum(abs.(fields.r_Vy)),
                    maximum(abs.(fields.∇V)),
                )
                ...
                update_error_plots!(ax.errs,err_plots,iter_evo,errs_evo)
                update_heatmaps!(heatmaps,fields)
                yield()
                # p1=heatmap(log10.(fields.η)',aspect_ratio=1)
                # p2=heatmap(fields.phase' .* fields.Pr',aspect_ratio=1)
                # p3=heatmap(fields.phase' .* fields.Vmag',aspect_ratio=1)
                # p4=heatmap(fields.phase' .* fields.τII',aspect_ratio=1)
