...
default(size=(800,500),framestyle=:box,label=false,grid=false,margin=10mm,lw=6,labelfontsize=11,tickfontsize=11,titlefontsize=11)
...
@views function update_stresses!((;εxx_ve,εyy_ve,εxy_ve,εII_ve,Pr,Pr_c,εxx,εyy,εxy,εxyv,εII,dτxx,dτyy,dτxy,τxx,τyy,τxy,τxyv,τxx_old,τyy_old,τxy_old,Vx,Vy,∇V,η_veτ,η,Gdτ,G,ηb,F,λ,dQdτxx,dQdτyy,dQdτxy,τII,η_vep,Fchk,dPr,Pr_old),K,τ_y,sinϕ,sinψ,η_reg,r,relλ,dt,dx,dy)
    ...
    # dQdτxy .=      (τxy.+dτxy)./τII
    Pr_c   .= Pr #.+ K.*dt.*λ.*sinψ
    τxx   .+= (.-(τxx .- τxx_old)./(G.*dt) .- τxx./η .+ 2.0.*(εxx .-      λ.*dQdτxx.*0.0)).*η_veτ
    τyy   .+= (.-(τyy .- τyy_old)./(G.*dt) .- τyy./η .+ 2.0.*(εyy .-      λ.*dQdτyy.*0.0)).*η_veτ
    τxy   .+= (.-(τxy .- τxy_old)./(G.*dt) .- τxy./η .+ 2.0.*(εxy .- 0.5.*λ.*dQdτxy.*0.0)).*η_veτ
...
@views function update_velocities!((;Vx,Vy,Pr_c,τxx,τyy,τxyv,dτ_ρx,dτ_ρy,η,ρgx,ρgy,phase),dx,dy)
    Vx[2:end-1,:] .+= dτ_ρx.*(diff(.-Pr_c.+τxx,dims=1)./dx .+ diff(τxyv[2:end-1,:],dims=2)./dy .- ρgx)
    Vy[:,2:end-1] .+= dτ_ρy.*(diff(.-Pr_c.+τyy,dims=2)./dy .+ diff(τxyv[:,2:end-1],dims=1)./dx .- ρgy)
    Vx[:,1]       .= 1.0./3.0.*Vx[:,2]
...
@views function compte_η_G_ρg!((;η,G,ρgy_c,phase,ηb),η0,G0,ρg0,xc,yc,x0,y0c,y0d,r_cav,r_dep,δsd)
    Threads.@threads for iy in axes(η,2)
        for ix in axes(η,1)
            sd_air = min(sqrt((xc[ix]-x0)^2 + 2*(yc[iy]-y0c)^2)-r_cav,
                         sqrt((xc[ix]-x0)^2 + 5*(yc[iy]-y0d)^2)-r_dep)
            t_air  = 0.5*(tanh(-sd_air/δsd) + 1)
            t_ice  = 1.0 - t_air
...
function main()
    ...
    # initialisation
    compte_η_G_ρg!(fields,η0,G0,ρg0,xc,yc,x0,y0c,y0d,r_cav,r_dep,δsd)
    ...
    # rad       = xc.^2 .+ yc'.^2
    # fields.G[rad.<rad2] .= Gi
    # fields.Vx   .=   εbg.*Xvx
    # fields.Vy   .= .-εbg.*Yvy
    # display(heatmap(fields.G',c=:turbo)); error("stop")
